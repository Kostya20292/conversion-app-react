...
- **On-surface muted (#9AA3B5):** вторичный текст, подписи, метаданные (размер файла, TTL, префикс
  API-ключа) — используется в вёрстке.
- **Secondary (#8B95A8):** Slate Mist — резервный оттенок той же роли; в коде не применяется, чтобы
  вторичный текст не разъезжался на два серых.
